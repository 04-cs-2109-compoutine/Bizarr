process.env.FIRESTORE_API_KEY = "AIzaSyAbids5u5V37mkGJ-cPeoxS2bUjUBnduSg";
process.env.FIRESTORE_AUTH_DOMAIN = "bizarr-11606.firebaseapp.com";
process.env.FIRESTORE_DATABASE_URL =
  "https://bizarr-11606-default-rtdb.firebaseio.com";
process.env.FIRESTORE_PROJECT_ID = "bizarr-11606";
process.env.FIRESTORE_STORAGE_BUCKET = "bizarr-11606.appspot.com";
process.env.FIRESTORE_MESSAGING_SENDER_ID = "623928421389";
process.env.FIRESTORE_APP_ID = "1:623928421389:web:5d9b295d4cf5e47ee87944";

process.env.GOOGLEMAPS_API_KEY = "AIzaSyDK6vhdXzcy1ZF27VFN3b1pqSQRNPTakYQ";

/*
use const GOOGLEMAPS_API_KEY = process.env.GOOGLEMAPS_API_KEY after importing to use
"config": {
  "googleMapsApiKey": "AIzaSyDK6vhdXzcy1ZF27VFN3b1pqSQRNPTakYQ"
},
*/
